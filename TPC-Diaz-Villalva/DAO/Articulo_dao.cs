...
namespace DAO
{
    public class Articulo_dao
    {
        ...
        {
            delArticuloVenta(articulo);
           
            string consulta = "DELETE FROM  ARTICULOS where Id=" + articulo.ID.ToString() + "";
        ...
        public void delArticuloVenta(Articulo articulo)
        {
            string consulta = "DELETE FROM  ARTICULOS_X_VENTAS where ID_ARTICULO=" + articulo.ID.ToString() + "";
            int filas = accesoDatos.EjecutarConsulta(consulta);
        ...
        public bool bajaLogicaArticulo(string id, bool eliminar)
        {
            int valor = 1;
            ...
            string consulta = "UPDATE ARTICULOS SET VISIBLE= '" + valor + "' WHERE ID = '"+ id +"'" ;
            int filas = accesoDatos.EjecutarConsulta(consulta);
        ...
        public List<Articulo> getArticulosFiltrados(string valor, bool tipo)
        {
            ...
            if (tipo)
            {
                 consulta = "SELECT ARTICULOS.ID,  ARTICULOS.NOMBRE, ARTICULOS.PRECIO,  ARTICULOS.IMAGEN,  ARTICULOS.STOCK,  CATEGORIAS.NOMBRE AS CATEGORIA,  COMPOSICIONES.NOMBRE AS COMPOSICION,  ISNULL(MEDIDAS.NOMBRE, MEDIDAS.ANCHO_CM + 'x' + MEDIDAS.LARGO_CM) AS MEDIDA, ESTILOS.NOMBRE AS ESTILO,  COLORES.CODIGO AS COLOR, ARTICULOS.VISIBLE FROM ARTICULOS INNER JOIN MEDIDAS ON MEDIDAS.ID = ARTICULOS.ID_MEDIDA INNER JOIN CATEGORIAS ON CATEGORIAS.ID = ARTICULOS.ID_CATEGORIA INNER JOIN COMPOSICIONES ON COMPOSICIONES.ID = ARTICULOS.ID_COMPOSICION INNER JOIN ESTILOS ON ESTILOS.ID = ARTICULOS.ID_ESTILO INNER JOIN COLORES ON COLORES.ID = ARTICULOS.ID_COLOR WHERE ARTICULOS.NOMBRE LIKE '%" + valor + "%' OR CATEGORIAS.NOMBRE LIKE '%" + valor + "%' OR MEDIDAS.NOMBRE LIKE '%" + valor + "%' OR ESTILOS.NOMBRE LIKE '%" + valor + "%' ";

            }

            else
            {
                 consulta = "SELECT ARTICULOS.ID,  ARTICULOS.NOMBRE, ARTICULOS.PRECIO,  ARTICULOS.IMAGEN,  ARTICULOS.STOCK,  CATEGORIAS.NOMBRE AS CATEGORIA,  COMPOSICIONES.NOMBRE AS COMPOSICION,  ISNULL(MEDIDAS.NOMBRE, MEDIDAS.ANCHO_CM + 'x' + MEDIDAS.LARGO_CM) AS MEDIDA, ESTILOS.NOMBRE AS ESTILO,  COLORES.CODIGO AS COLOR, ARTICULOS.VISIBLE FROM ARTICULOS INNER JOIN MEDIDAS ON MEDIDAS.ID = ARTICULOS.ID_MEDIDA INNER JOIN CATEGORIAS ON CATEGORIAS.ID = ARTICULOS.ID_CATEGORIA INNER JOIN COMPOSICIONES ON COMPOSICIONES.ID = ARTICULOS.ID_COMPOSICION INNER JOIN ESTILOS ON ESTILOS.ID = ARTICULOS.ID_ESTILO INNER JOIN COLORES ON COLORES.ID = ARTICULOS.ID_COLOR " + valor;
            }
            ...
            for (int i = 0; i < tabla.Rows.Count; i++)
            {
                Articulo art = new Articulo
                {
                    ...
                    visible= Convert.ToBoolean(tabla.Rows[i][10])
                };
